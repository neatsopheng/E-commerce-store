...
const Sidebar = () => {
  ...
  const {setCategory, productQuery, setProductQueryNull} = useProductQueryStore();
  ...
  return (
    <div className="md:w-64 md:fixed md:left-0 md:top-0 hidden md:block mt-20  bg-slate-500 h-screen p-5 text-white overflow-y-auto">
      ...
      <ul>
        <li className="w-full bg-slate-400 text-black text-xl cursor-pointer h-12 my-2 flex items-center pl-1"
          onClick={() => setCategory(undefined!)}
        >All</li>
        {data?.map((c) => (
          <li key={c} className="w-full bg-slate-400 text-black text-xl cursor-pointer h-12 my-2 flex items-center pl-1" value={c}
